...
#define FLSEC0		0x100000
#define FLSEC1		0x200000
#define FLSEC2		0x300000
#define FLSEC3		0x400000
#define FLSEC4		0x500000
#define FLSEC5		0x600000
...
void dump_memory(unsigned long segment, unsigned offset)
{
#define SIZEBUF 128
unsigned char xhuge *dpt;
unsigned char buf[SIZEBUF], copy[SIZEBUF]; 
unsigned counter, i=0, j=0, k=0;
unsigned long address;
unsigned char xhuge *pt, ch;

address = segment+offset;			// calculate absolute address
dpt=(unsigned char xhuge *)0x100000;		// can point to anything
pt =(unsigned char xhuge *)copy;		// copy to use for ascii output
xmemcpy(buf, (unsigned xhuge *)address, 128);
xmemcpy(copy, buf, 128);
printf("\n\n\nDumping memory from %x\n", 0x100000);
delay(4);
...
while(counter > 0)
	...
	if (i%8==0) 
		...
		if (i > 8) 
			for (j=0; j<16; j++) 
				{
 			    ch = pt[j + (i*2)];	// examine ascii output
...
ADDRSEL3= 0x3007;		// sets Flash start address to 
				// 0x300000 and block size 512K Byte
...
puts(	"\nC167 monitor and setup options\n"
	...
	"2. Unused option.\n"
	"3. Erase flash memory block at 0x10000H.\n"
	"4. Write flash data to 0x100000H.\n"
	"5. Read flash address 0x100000H.\n"
	"6. Display setup and hardware information.\n"   
	"7. Dump memory address 0x100000H.\n"
...
while(!strchr(_rx_byte, replies)) 	
	...
	switch(_rx_byte)
		...
		case 0x32:	
			...
			break;
		case 0x33:	
			 status = PFlash_Erase(0x100000);
			printf("\nErasing flash memory at 0x100000H. (Code: %xh)", status); 
			prompt();
			S0RBUF=0;	
			_rx_byte=0;	
			break;
		case 0x34: 	
			status = PFlash_Write((void huge *)0x100000, (void huge *)(fl_name+6));
		...
		case 0x35:
			printf(	"\nReading ident string"
				"from 0x100000H: %20s.", (char huge *)0x100000);
			prompt();
			S0RBUF=0;  
			_rx_byte=0;
			break;
