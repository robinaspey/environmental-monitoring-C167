...
dpt=(unsigned short *)address;
pt =(char xhuge *)copy;
xmemcpy(buf, (unsigned short xhuge *)address, 64);
xmemcpy(copy, buf, 64);
printf("\n\n\nDumping memory from %04x\n", address);
counter=64;				// needs to be initialised here for some reason
while(counter > 0)
	{
	counter--;
	if (i%8==0) 
...
void monitor(void)
{
unsigned char const fl_name[64]={"C167 FL/512K XXXX"};
